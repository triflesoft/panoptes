...
function do_provision {
    ...
    case $OS_ID in
        rhel-10.1)
            ...
            dnf --assumeyes --quiet install certbot curl dnf-automatic dnsmasq freeradius freeradius-rest freeradius-utils git glibc-langpack-en htop iotop jq lsof nano nftables openvpn openssl patch policycoreutils-python-utils polkit python3-certbot-dns-route53 rsync strongswan strongswan-sqlite tar telnet tmux traceroute unzip valkey wget
            ;;
    esac
    ...
    curl --output /tmp/panoptes.zip --location https://github.com/triflesoft/panoptes/archive/refs/heads/main.zip
    unzip -oK /tmp/panoptes.zip -d /tmp/panoptes
    mv --force /tmp/panoptes/panoptes-main/* /opt/panoptes
    ls -laZ /opt/panoptes/
    ...
    /opt/panoptes/provision/provision.sh
}
