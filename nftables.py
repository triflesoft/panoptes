    ...
    def download(self):
        if not isfile(self.ripe_database_file_path):
            ...
            with request("GET", self.ripe_database_url, stream=True) as response:
                ...
                with open(self.ripe_database_file_path + "~", mode="wb") as file:
                    read_length = 0
            ...
            rename(self.ripe_database_file_path + "~", self.ripe_database_file_path)
            print("\r\033[KSaving database... done")

    def parse(self):
        if not isfile(self.ripe_catalog_file_path):
            line_index = 0
            ...
            with open(self.ripe_database_file_path, "rb") as ripe_gzip_file:
                ...
                rename(self.ripe_catalog_file_path + "~", self.ripe_catalog_file_path)

                print("\r\033[KSaving catalog... done")

    def build(self):
        nftables_ipv4_config_path = join(self.source_path, "etc/sysconfig/nftables-geoip-ipv4.nft.r440")
        nftables_ipv6_config_path = join(self.source_path, "etc/sysconfig/nftables-geoip-ipv6.nft.r440")

        if (not isfile(nftables_ipv4_config_path)) or (not isfile(nftables_ipv6_config_path)):
            ...
            for country_code, country_data in catalog.items():
                ...
                print(f"\r\033[KProcessing country {country_code}... done", flush=True)
            ...
            with open(nftables_ipv4_config_path, "wb") as nftables_ipv4_config_file:
                with open(nftables_ipv6_config_path, "wb") as nftables_ipv6_config_file:
